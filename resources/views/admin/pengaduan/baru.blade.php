                            ...
                            <div class="mt-6 space-y-4">
                                ...
                                @if ($yazid_item->status == '0')
                                    <div class="flex justify-between items-center space-x-2">
                                        <form action="/admin/{{ $yazid_item->id_pengaduan }}/terima" method="POST"
                                            class="w-1/2">
                                            @csrf
                                            <button type="submit"
...
@else
    ...
    <script src="{{ asset('js/filter-pengaduan.js') }}"></script>
    <script>
        ...
        `;
            ...
            fetch(`/api/pengaduan/${id_pengaduan}`)
    ...
    <script>
        function showResponseForm(id_pengaduan) {
            ...
            fetch(`/admin/tanggapan/${id_pengaduan}`)
                .then(response => response.json())
                .then(data => {
                    tanggapanList.innerHTML = '';
                    if (data.length > 0) {
    ...
    <script>
        ...
        document.getElementById('confirmButton').addEventListener('click', function() {
            ...
            form.method = 'POST';
            form.action =
                `/admin/${currentId}/${currentAction}`; // Pastikan endpoint ini sesuai dengan route untuk menghapus pengaduan
