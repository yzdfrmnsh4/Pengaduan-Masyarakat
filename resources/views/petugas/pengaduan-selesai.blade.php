<x-layout>
    <div class="p-6 sm:p-8 relative min-h-screen">
        <h1 class="text-2xl font-semibold text-gray-800 mb-6">Pengaduan Selesai</h1>
        <form action="{{ route('petugas.pengaduan.done') }}" method="GET">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-lg hover:shadow-xl transition duration-300 transform hover:-translate-y-1">
                <div class="relative h-56 overflow-hidden">
                    @if($yazid_item->foto)
                        <img src="{{ asset('storage/' . $yazid_item->foto) }}" alt="Foto Pengaduan" class="w-full h-full object-cover transition duration-300 transform hover:scale-105">
                    @else
                ...
                <div class="p-6">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 bg-blue-500 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-md">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500">{{ \Carbon\Carbon::parse($yazid_item->tgl_pengaduan)->format('d M Y') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 space-y-4">
                        <div class="flex items-center justify-between">
                            <button onclick="openDetailModal('{{ $yazid_item->id_pengaduan }}')" class="text-blue-600 hover:text-blue-700 text-xs font-medium flex items-center transition duration-300 hover:translate-x-1">
                            ...
                            <div class="flex space-x-2">
                                <button onclick="showResponseForm({{ $yazid_item->id_pengaduan }})" class="px-4 py-2 bg-blue-500 text-white text-sm font-medium rounded-lg hover:bg-blue-600 transition duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
                                    Tanggapi
                                </button>
                            </div>
                        </div>
...
<script>
    function showResponseForm(id_pengaduan) {
        ...
        tanggapanList.innerHTML = '<div class="flex justify-center"><svg class="animate-spin h-8 w-8 text-blue-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></div>';
